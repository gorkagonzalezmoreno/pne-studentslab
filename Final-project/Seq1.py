...
class Seq1:
    ...
    def read_fasta(self, filename):
        content = Path(filename).read_text() #Path(filename) crea un objeto de tipo Path y le paso el fichero que voy a abrir
        lines = content.splitlines() #el splitlines separa automaticamente por defecto en cada \n
        body = lines[1:]
        self.bases = ""
        for line in body:
            self.bases += line
    ...
    def most_frequent_base(self):
        if self.bases == "NULL" or self.bases == "ERROR":
            ...
            result = None
            for base, count in self.count().items(): #en base guardo la letra y en count el numero de veces que aparece
                if result is None: #if not result:
                    result = {"base": base, "count": count}
                else:
                    if count > result["count"]:# count > que el contenido de la clave count?
                        result["base"] = base #asocio el nuvo valor, ya que es mayor
                        #result = {"base": base, "count": count} machaco el diccionario anterior y lo pongo tood igual de nuevo

            return result["base"] #devuelvo solo la base mas repetida, el contador ya da igual , ya lo he comparado
    ...
    def info(self):
        ...
        result += f"Most frequent base: {self.most_frequent_base()}"
        return result
